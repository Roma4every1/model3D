...
export default function Maps(provider) {
	...
	this.loadMap = async (mapID) => {
		const ret = await provider.getMap(mapID);
		...
		ret.layers.forEach((layer) => {
			if (typeof layer.lowscale === 'string' && layer.lowscale !== 'INF')
				layer.lowscale = parseInt(layer.lowscale);
			if (typeof layer.highscale === 'string' && layer.highscale !== 'INF')
				layer.highscale = parseInt(layer.highscale);
		});

		try {
			const data = await provider.getContainer(ret.namedpoints);
			ret.points = data.namedpoints;
		} catch (error) {
		...
		for (let layer of ret.layers) {
			layer.elements = null;
				...
				try {
					...
					const data = await provider.getContainer(layer.container, indexName);
					const layerFromContainer = layer.uid.includes(layer.container)
						? data.layers[layer.uid.replace(layer.container, '')]
						: data.layers[layer.uid];
					...
					if (elements.length === 0) {
						...
						const newLayer = Object
							.values(data.layers)
							.find((l) => l.name === nameFromContainerInBrackets);
						if (newLayer != null) {
					...
					for (let i of elements) {
						...
						if (t && t.loaded) await t.loaded(i, provider);
					}
				} finally {
					layer.elements = elements;
				}
