...
const middle = (array) => sum(array) / array.length;
const norm = (array) => { const m = sum(array); return array.map(a => a / m) };

const middleCoord = (coords) => {
...
export var rects = {
	...
	middleRect: (...rects) => {
		return {
			min: {x: middleCoord(rects.map(r => r.min.x)), y: middleCoord(rects.map(r => r.min.y))},
			max: {x: middleCoord(rects.map(r => r.max.x)), y: middleCoord(rects.map(r => r.max.y))},
		};
	},
}
...
const translate = (scale1, p1, scale2, p2) => {
	...
	return (point) => ({x: p2.x + (point.x - p1.x) * sc, y: p2.y + (point.y - p1.y) * sc});
}
...
export function translator(mScale, mCenter, cScale, cCenter) {
	...
	return ret = {
		...
		changeResolution: (mul) => {
			return mul === 1
				? ret
				: translator(mScale, ret.pointToMap({ x: 0, y: 0 }), cScale * mul, { x: 0, y: 0 })
		},
	}
}
