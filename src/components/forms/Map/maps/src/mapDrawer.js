...
var namedpoint = declareType( "namedpoint", {
	bound: pointBounds,
...
// function dashedLine( context, from, to, pattern, width ) {
// 	var dx = to.x - from.x
// 	var dy = to.y - from.y
...
// 	var p = 0
// 	while ( d < length ) {
// 		var s = Math.min( 1, d / length )
// 		context.moveTo( x + dx * s, y + dy * s )
// 		d += width * pattern[ p++ ]
// 		var s = Math.min( 1, d / length )
// 		context.lineTo( x + dx * s, y + dy * s )
// 		d += width * pattern[ p++ ]
// 		if ( p >= pattern.length )
// 			p = 0
// 	}
...
var field = declareType( "field", {
	...
	getStocksWithinContour: (i, options) => {
		...
		if (options.contourBuildRequired) {
			...
			let contourOptions = options.contourOptions || {};
	...
	_draw: ( i, options ) => {
		...
		// get the visible part of data array
		let fieldBounds = {
			min: options.pointToControl({
				x: Math.min.apply( Math, [ i.x, i.x + i.sizex * i.stepx ] ),
				y: Math.min.apply( Math, [ i.y - i.sizey * i.stepy, i.y ] )
			}),
			max: options.pointToControl({
				x: Math.max.apply( Math, [ i.x, i.x + i.sizex * i.stepx ] ),
				y: Math.max.apply( Math, [ i.y - i.sizey * i.stepy, i.y ] )
			})
		};
		let screenDrawBounds = {
			min : options.pointToControl(options.drawBounds.min),
			max : options.pointToControl(options.drawBounds.max)
		};
	...
	decorationPath: function ( i, options, lineConfig ) {
		var fillSegmentWithDecoration = function (ctx, lineConfig, point, lpoint, overhead, i, options) {
			...
			for (var k = decorations.length - 1; k >= 0; k--) {
				...
				//var i = decoration.initialInterval._value + overhead;
				var i = 0;
				...
				//double.TryParse(number.Replace(".", ","))
	...
	draw: function* drawThread( i, options ) {
		...
		if ( i.style ) {
			...
			if (i.style.StrokeDashArrays) {
				...
				if (dashObj.onBase._value) {
					var dashes = dashObj.data._value.split(" ");
					for (var j = dashes.length - 1; j >= 0; j--) {
	...
	*/
	...
	return startThread( function* paintThread() {
		...
		try {
			...
			if ( !layer.elements ) {
				var r = onDataWaiting( layer.elementsData );
				if ( r )
					yield r;
			}
			if ( layer.elements ) for ( var element of layer.elements ) {
				if ( total == 0 )
					L.info( "type of first element in sublayer is", element.type );
				++total;
				++mapTotal;
				var D = types[ element.type ];
			...
			var r = onDataWaiting( map.pointsData );
			if ( r )
				yield r;
		}
			if ( map.points && drawOptions.selected && drawOptions.selected.indexOf ) {
			var D = types[ "namedpoint" ];
			if ( D ) for ( var element of map.points ) {
				if ( !geom.rects.intersects( drawBounds, D.bound( element ) ) )
					continue;
