...
//import mydrawer from './maps/src/mapDrawer.js';
var mydrawer = require("./maps/src/mapDrawer");
var geom = require("./maps/src/geom");
var pixelPerMeter = require("./maps/src/pixelPerMeter");
var cache = require("./maps/src/cache");
var htmlHelper = require( "./maps/src/htmlHelper" )
var startThread = require( "./maps/src/startThread" )
var logger = require( "./maps/src/logger" )
var symbol2svg = require( "./maps/src/symbol2svg" )
var cp1251 = require( "./maps/src/cp1251" )
var def2json = require( "./maps/src/def2json" )
...
class App extends React.Component {
	...
	var getSymbolsLib = cache( () => this.httpClient.getHTTP( SERVER_URL + "/ij-srv/libs/symbol.def", "binary" ).then(
		text => symbol2svg( def2json( cp1251.toUnicode( text ) ) )
	) )

	var getImage = cache( ( name, index, color ) => {
		return startThread( function* () {
			var lib = yield getSymbolsLib()
...
if (bounds)
{
 ...
 localy = (bounds[0].min.y + bounds[0].max.y) / 2;
    ...
    updateCanvas() {
        ...
        var mymap = {layers: [{name: "Layer1Test", visible: true, elements: [{type: "pieslice", x: 220, y: 210, radius: 15, color: "green", startangle: 130 / 180  * Math.PI, endangle: 10 / 180 * Math.PI }, 
