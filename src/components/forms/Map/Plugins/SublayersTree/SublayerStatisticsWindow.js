...
export default function SublayerStatisticsWindow(props) {
    const { t } = useTranslation();
    const dispatch = useDispatch();
    const { sublayer, header } = props;
    const [windowSize, setWindowSize] = React.useState({ width: 300, height: 110 });
    ...
    const translateType = React.useCallback((type) => {
        switch (type) {
            case 'label':
                return t("map.labels");
            case 'polyline':
    ...
    return (
        ...
        >
            ...
            <DialogActionsBar>
                <Button className="actionbutton" onClick={close}>
                    {t('base.ok')}
                </Button>
            </DialogActionsBar>
        </Window>
