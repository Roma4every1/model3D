...
export default function Grid({formData: {id: formID}}) {
  ...
  const formChildrenState = useSelector(selectors.formChildrenState.bind(formID));
  const layout = useSelector(selectors.formLayout.bind(formID));
  ...
  }, [formID, formChildrenState, dispatch]);
  ...
  useEffect(() => {
    ...
    if (layout) {
      ...
      if (formChildrenState && openedForms && !ignore) {
        const fetchData = async () => {
          const data = await sessionManager.fetchData(`getFormLayout?sessionId=${sessionID}&formId=${formID}`);
    ...
    return () => { ignore = true; }
  }, [formChildrenState, sessionID, formID, openedForms, layout, correctElement, sessionManager]);

  if (!openedForms) return <Loader size={'small'} type={'infinite-spinner'} />;
  return (
    <Container formID={formID} modelJson={modelJson}>
