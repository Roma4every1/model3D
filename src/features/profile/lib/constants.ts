// /** Размер шага вдоль трассы для построения профиля. */
// export const PROFILE_X_STEP = 200;
//
// /** Настройки отрисовки. */
// export const drawerConfig: ProfileDrawerConfig = {
//   stage: {
//     padding: 4,
//     font: {size: 14, style: 'normal', family: '"Segoe UI", Roboto, sans-serif'}
//   },
//   axis: {
//     backgroundColor: '#EEEEEE',
//     color: '#000000',
//     font: {size: 24},
//     markSize: 10
//   },
// };

export const GMMOJobDefaultParams = {
  organizationCode: 'dbmm_tat$1',
  resExt: 'xml',
  reqExt: 'xml',
  builderId: '{E2BB9801-C8B0-4869-AA9F-E1D136CB479E}'
}

export const GMMOBuilderParameters = 'PEJ1aWxkZXJQYXJhbWV0ZXJzPg0KICA8UGFyYW0gbm' +
  'FtZT0iUHJvZmlsZVNjYWxlIiB2YWx1ZT0iNSIgLz4NCiAgPFBhcmFtIG5hbWU9IldPQ0xldmVsIiB2YWx1ZT0' +
  'iMCIgLz4NCiAgPFBhcmFtIG5hbWU9IldPQ0xldmVsQ2hlY2tlZCIgdmFsdWU9IkZhbHNlIiAvPg0KICA8UGFy' +
  'YW0gbmFtZT0iUGFsZW9Qcm9maWxlRW5hYmxlZCIgdmFsdWU9IkZhbHNlIiAvPg0KICA8UGFyYW0gbmFtZT0iU' +
  'GFsZW9Qcm9maWxlUEwiIHZhbHVlPSI2MDYiIC8_DQogIDxQYXJhbSBuYW1lPSJQYWxlb1Byb2ZpbGVUb3BCYX' +
  'NlIiB2YWx1ZT0iMSIgLz4NCjwvQnVpbGRlclBhcmFtZXRlcnM_';
